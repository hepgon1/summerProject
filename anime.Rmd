...
cols = final_ds[sapply(final_ds, class) == "integer"]
corr = cor(final_ds[cols], method = "pearson")
```
...
prfTune= prfTune+geom_text(aes(label=
                     paste0("Mtry= ", rfTune$results$mtry,", Sensitivity = ",
...
pred = predict(rfTune, newdata = f.test)

rfpredtbl = table(pred,f.test$my_scr_bin)

rfSens = rfpredtbl[1,1] /(rfpredtbl[1,1] + rfpredtbl[1,2])
...
pknnTrain= pknnTrain+geom_text(aes(label=
                     paste0("k=", knnTune$results$k,", Sens=",
...
predict KNN
```{r}
knnpred = predict(knnTune, newdata = f.test)

knnpred
knnpredtbl = table(knnpred,f.test$my_scr_bin)

knnSens = knnpredtbl[1,1] /(knnpredtbl[1,1] + knnpredtbl[1,2])
...
```{r}
Sys.time()
logTune = train(my_scr_bin ~ ., data = f.train,
                 method = "glm", 
                 metric = "Sensitivity",
                 family="binomial",
                 na.action = na.exclude,
                 preProc = c("center", "scale") #, 
                 #trControl = trainControl(method = "cv", classProbs = T,summaryFunction = twoClassSummary))
)
Sys.time()
