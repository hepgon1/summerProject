...
Read back in the sample of the user to Anime File
```{r}
read.myUsrAni.file = function (){
  myUsrAni.file = paste0(wd, "/code/output/MycleanUserAnimeList.csv")
  usrAni = read.csv(myUsrAni.file, sep = ",", header = T)
  return (usrAni)
}
...
```{r}
read.Ani.file = function (){
  myAni.file = paste0(wd, "/myAnimeList Dataset/extracted data/UsrList.csv")
  Ani = read.csv(myAni.file, sep = ",", header = T)
  return (Ani)
}
```
