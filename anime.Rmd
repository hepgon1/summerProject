...
corrplot(corr, method = "ellipse", type = "lower", tl.srt = 45)
...
```{r}
# Sys.time()
# stepTune = train(my_scr_bin ~ ., data = f.train,
#                  method = "glmStepAIC",
#                  direction = "both",
#                  metric = "Sensitivity",
#                  family="binomial",
...
```{r}
# stepTune
# colnames(stepTune$finalModel$model)
```


Model input
```{r}
model = my_scr_bin ~ anime_src + anime_len + rate + score + rank + popularity + favorites + user_watching + user_completed + user_onhold + user_dropped + user_plantowatch + lst_online_yr + join_yr + gnd_grp + broadcasted
model
```
...
```{r}
logpred = predict.train(logTune, newdata = f.test)

confusionMatrix(logpred,f.test$my_scr_bin)
```
...
```{r}
Sys.time()
ldaTune = train(model, data = f.train,
                method = "lda",
                metric = "Sensitivity",
                preProc = c("center", "scale"),
                tuneLength = 10,
...
ldaPred = predict(ldaTune, f.test)

confusionMatrix(ldaPred,f.test$my_scr_bin)
```

SVM
```{r}
# Sys.time()
# svmTune = train(model, data = f.train,
#                 method = "svmRadial",
#                 #preProc = c("center", "scale"),
#                 tuneLength = 10,
#                 trControl = trainControl(method = "cv", 
...
# #svmTune
# str(svmTune)
#colnames(svmTune$finalModel$model)
```

```{r}
# str(svmPredict)
# #to get class probabilities
# svmProbs <- predict(svmTune, newdata = f.test,
#                     type = "prob") 
# head(svmProbs)
```
