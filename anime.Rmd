...
My status
```{r}
sqldf("select distinct my_status from all_ds order by my_status")
all_ds$my_stat = as.factor(all_ds$my_status)
summary(all_ds$my_stat)
```
Rank
...
```{r}
corr = cor(final_ds, method = "pearson")
```
...
final_ds = ds[, c("my_scr_bin", "type", "anime_src", "episodes", "anime_len", "rating", "score", "scored_by", "rank", "popularity", "members", "favorites", "my_watched_episodes_int", "my_stat_int", "user_watching", "user_completed", "user_onhold", "user_dropped", "user_plantowatch", "lst_online_yr", "join_yr", "gnd_grp", "related_by")]
summary(final_ds)
str(final_ds)
head(final_ds)
...
rf = train(my_scr_bin ~ ., data=f.train, 
                    method="rf", 
...
```{r}
f.spam.rf = function(data, mtries, nrtrees) {
set.seed (1)
  rf.bag.spam =randomForest(my_scr_bin ~ .,data=data, mtry= mtries, ntree=nrtrees, importance =TRUE)
  rf.bag.spam
  list(rf.spam = rf.bag.spam, mtries = mtries, nrtrees = nrtrees, call = rf.bag.spam$call)
}

get.err.rt = function(nrtrees) {
  OOB=NULL
  spam.rf = NULL
  for (i in 4:6) {
    spam.rf = f.spam.rf(f.train, i, nrtrees)
    OOB[i] = round(tail(spam.rf$rf.spam$err.rate, n=1)[1]*100, 2)
  }
  min.OOB = min(OOB, na.rm = T)
  min.OOB
  best.err.rt = which(min.OOB==OOB)
  best.err.rt
  list(min.oob = min.OOB, best.err.rt=best.err.rt, funct = spam.rf)
}

spam.rf.500 = get.err.rt(50)
spam.rf.500$best.err.rt
spam.rf.500$min.oob

spam.rf.1000 = get.err.rt(100)
spam.rf.1000$best.err.rt
spam.rf.1000$min.oob

spam.rf.2000 = get.err.rt(200)
spam.rf.2000$best.err.rt
spam.rf.2000$min.oob
...
predict RF
```{r}
pred = predict(rf, newdata = f.test)
...
```{r}
Sys.time()
knnTune = train(my_scr_bin ~ ., data = f.train,
                 method = "knn", 
                 na.action = na.exclude,
